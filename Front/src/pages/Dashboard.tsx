...
import { Plus, Trash2, CheckCircle, Circle, Clock, Edit2, X, Filter, CheckSquare } from 'lucide-react';
...
const fetchTasks = async () => {
    ...
    const handleUpdateTask = async (taskId: number, updates: Partial<Pick<Task, 'title' | 'description' | 'status'>>) => {
        try {
...
return (
        <div className="space-y-6">
            ...
            <div className="flex items-center space-x-2">
                ...
                <select
                    value={statusFilter}
                    onChange={(e) => setStatusFilter(e.target.value as 'all' | 'pending' | 'in_progress' | 'done')}
...
{loading ? (
                ...
                <div className="text-center py-12 bg-white rounded-lg border-2 border-dashed border-gray-300">
                    ...
                    <h3 className="mt-2 text-sm font-medium text-gray-900">
                        {statusFilter === 'all' ? 'No tasks' : `No ${statusFilter.replace('_', ' ')} tasks`}
                    </h3>
                            ...
                            <li key={task.id}>
                                {editingTask?.id === task.id ? (
                                    ...
                                    <div className="px-4 py-4 sm:px-6 hover:bg-gray-50 transition-colors flex items-center justify-between group">
                                        <div className="flex items-center min-w-0 flex-1 cursor-pointer" onClick={() => handleUpdateStatus(task)}>
                                            <div className="flex-shrink-0 mr-4">
                                                {getStatusIcon(task.status)}
                                            </div>
                                            <div className="min-w-0 flex-1">
                                                <p className={`text-sm font-medium truncate ${task.status === 'done' ? 'text-gray-500 line-through' : 'text-indigo-600'}`}>
                                                    {task.title}
                                                </p>
                                                {task.description && (
